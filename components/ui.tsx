                ...
                <Upload onHandleFiles={(files: FileList | null) => handleFiles(files)} />
                {
                    userFiles.length > 0 ? (
                        <>
                            ...
                            />
                            <Buttons loading={loading} convert={() => convert()} preview={() => preview()} />
                            <Table isConverted={isConverted} files={userFiles} downloads={() => downloadImages()} download={(item: userFile) => downloadImage(item)} clear={() => setUserFiles([])} />
                        </>
                    ) : null
                }
                <FAQ />
